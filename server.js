...
if (twilioConfig.accountSid && twilioConfig.authToken) {
   ...
   console.warn('⚠️ Configuración de Twilio incompleta');
}

// Servicio de notificaciones SMS
class SMSNotificationService {
    constructor() {
        this.lastNotifications = new Map(); // Para evitar spam
        this.THROTTLE_TIME = 10000; // 10 segundos entre notificaciones del mismo tipo
    }

    // Verificar si se puede enviar la notificación (throttling)
    canSendNotification(key) {
        const lastSent = this.lastNotifications.get(key);
        if (lastSent && (Date.now() - lastSent) < this.THROTTLE_TIME) {
            return false;
        }
        return true;
    }
...
_¡Excelente juego!_`;
    ...
    // Enviar SMS usando Twilio - ACTUALIZADO CON MODO PRUEBA
    async sendSMS(message, notificationKey = null) {
        // NUEVO: Modo de prueba cuando se alcanza el límite diario
        const isTestMode = process.env.SMS_TEST_MODE === 'true';
        
        if (!twilioClient && !isTestMode) {
            console.log('❌ Cliente de Twilio no disponible');
            return { success: false, error: 'Cliente de Twilio no configurado' };
        }

        // Verificar throttling si se proporciona una clave
        if (notificationKey) {
            if (!this.canSendNotification(notificationKey)) {
                console.log(`⏸️ Notificación throttled: ${notificationKey}`);
                return { success: false, reason: 'throttled' };
            }
        }

        // NUEVO: Modo de prueba - simular envío exitoso
        if (isTestMode) {
            console.log(`📱 [MODO PRUEBA] SMS simulado: ${message.substring(0, 50)}...`);
        ...
        try {
            const messageResult = await twilioClient.messages.create({
                ...
                to: twilioConfig.toPhoneNumber
            });

            console.log(`✅ SMS enviado exitosamente: ${messageResult.sid}`);
            
            // Marcar como enviado si tiene clave
            if (notificationKey) {
                this.markNotificationSent(notificationKey);
            }
            ...
            // Manejo especial para límite diario de Twilio
            if (error.code === 63038) {
                console.log('🚫 Límite diario de SMS alcanzado en Twilio - activando modo prueba automáticamente');
                
                // Activar automáticamente el modo de prueba
                process.env.SMS_TEST_MODE = 'true';
                
                // Reenviar en modo de prueba
                return await this.sendSMS(message, notificationKey);
            }
    ...
    async notifyPlayerOnline(playerName, mesaType = "ROYAL") {
        ...
        const message = this.formatPlayerOnlineMessage(playerName, mesaType);
    ...
    // Notificar grandes ganancias
    async notifyBigWin(playerName, amount) {
        if (!playerName || playerName === 'admin' || amount <= 20000) {
            return { success: false, reason: 'criteria_not_met' };
        }

        const notificationKey = `big_win_${playerName}`;
        const message = this.formatBigWinMessage(playerName, amount);
        
        return await this.sendSMS(message, notificationKey);
    }

    // Notificar puntajes altos
    async notifyScoreMilestone(playerName, score) {
        if (!playerName || playerName === 'admin' || score < 100000) {
            return { success: false, reason: 'criteria_not_met' };
        }

        const notificationKey = `milestone_${playerName}_${Math.floor(score/50000)}`;
        const message = this.formatScoreMilestoneMessage(playerName, score);
        
        return await this.sendSMS(message, notificationKey);
    }

    // Enviar notificación personalizada
    async sendCustomNotification(message, notificationKey = null) {
        return await this.sendSMS(message, notificationKey);
    }
}
...
if (serviceAccount) {
  admin.initializeApp({
      ...
      databaseURL: process.env.FIREBASE_DATABASE_URL || "https://juego-memoria-quinto-default-rtdb.firebaseio.com/"
  });
  db = admin.database();
...
const USERS_FILE = path.join(__dirname, 'users.json'); // NUEVO: Archivo para usuarios
...
async function loadUsers() {
 ...
 // PASO 1: Intentar cargar desde Firebase primero
 if (db) {
   try {
     console.log('Intentando cargar usuarios desde Firebase...');
     const snapshot = await db.ref('users').once('value');
     const firebaseUsers = snapshot.val();
     
     if (firebaseUsers && typeof firebaseUsers === 'object') {
       // Convertir el objeto de Firebase a array
       loadedUsers = Object.values(firebaseUsers);
       console.log('Usuarios cargados desde Firebase:', loadedUsers.length);
     }
   } catch (firebaseError) {
     console.error('Error al cargar usuarios desde Firebase:', firebaseError);
   }
 }

 // PASO 2: Si no se pudo cargar desde Firebase, intentar desde archivo local
 if (!loadedUsers) {
   try {
     if (fs.existsSync(USERS_FILE)) {
       const usersData = fs.readFileSync(USERS_FILE, 'utf8');
       loadedUsers = JSON.parse(usersData);
       console.log('Usuarios cargados desde archivo local:', loadedUsers.length);
     }
   } catch (error) {
     console.error('Error al cargar usuarios desde archivo:', error);
   }
 }

 // PASO 3: Si todo falla, usar usuarios por defecto
 if (!loadedUsers || !Array.isArray(loadedUsers)) {
   console.warn('Usando usuarios por defecto');
   loadedUsers = [
     { id: '1', username: 'Sirius', password: '4598701', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '2', username: 'Vega', password: '3587412', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '3', username: 'Draco', password: '8632542', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '4', username: 'Cygnus', password: '6412590', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '5', username: 'Altair', password: '7523698', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '6', username: 'Rigel', password: '2234568', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '7', username: 'Spica', password: '8654322', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '8', username: 'Canopus', password: '2122335', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '9', username: 'Deneb', password: 'estrella456', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
     { id: '10', username: 'Polaris', password: 'norte123', score: 60000, prevScore: 60000, isAdmin: false, isBlocked: false, isLockedDueToScore: false },
...
async function saveUsers() {
 try {
   // PASO 1: Guardar en archivo local
   const usersData = JSON.stringify(users, null, 2);
   fs.writeFileSync(USERS_FILE, usersData);
   console.log('Usuarios guardados en archivo local');

   // PASO 2: Guardar en Firebase si está disponible
   if (db) {
     try {
       ...
       await db.ref('users').set(usersObject);
       console.log('Usuarios guardados en Firebase');
     } catch (firebaseError) {
       console.error('Error al guardar usuarios en Firebase:', firebaseError);
     }
   }
 } catch (error) {
...
// Variable para usuarios - se inicializará después
let users = [];
...
function checkScoreLimit(user) {
  ...
  if (user.score <= 23000 && !user.isAdmin) {
      ...
      if (!user.isLockedDueToScore) {
          ...
          saveGameState();
          saveUsers(); // NUEVO: Guardar usuarios después del cambio
      }
...
function verifyBlockingStates() {
  ...
  if (inconsistenciasCorregidas > 0) {
      ...
      saveUsers(); // NUEVO: Guardar usuarios después de correcciones
  }
...
async function saveGameState() {
  const stateToSave = {
      ...
      currentPlayerIndex: gameState.currentPlayerIndex || 0,status: gameState.status || 'playing',
...
// Cargar usuarios e inicializar estado del juego al iniciar el servidor
(async function () {
try {
    users = await loadUsers(); // Hacer la carga asíncrona
    console.log('Usuarios cargados al iniciar el servidor:', users.length);
...
// MODIFICADO: Generar el tablero con distribución aleatoria de fichas ganadoras y perdedoras en cada hilera
// Cambio: Los valores ahora son 20000 y -20000 en lugar de 30000 y -30000
...
for (let row = 0; row < 4; row++) {
    ...
    for (let i = 0; i < 2; i++) {
        rowTiles.push({ value: 10000, revealed: false });  // CAMBIADO: 20000 en lugar de 30000
    }
    for (let i = 0; i < 2; i++) {
        rowTiles.push({ value: -11000, revealed: false }); // CAMBIADO: -20000 en lugar de -30000
    }
...
for (let i = 0; i < gameState.board.length; i++) {
    ...
    if (!gameState.board[i] || gameState.board[i].value === undefined) {
        gameState.board[i] = {
            value: (Math.random() > 0.5 ? 10000 : -11000), // CAMBIADO: 20000 en lugar de 30000
            revealed: false
        };
        fichasCorregidas++;
    }
...
await saveGameState();
await saveUsers(); // NUEVO: Guardar usuarios después del reset
}
...
socket.on('joinGame', () => {
    ...
    if (existingPlayerIndex === -1) {
        ...
        // NUEVO: Notificar conexión del jugador (solo si no es admin)
        if (!user.isAdmin) {
            ...
            smsNotificationService.notifyPlayerOnline(user.username, "ROYAL")
                .then(result => {
                    if (result.success) {
                        if (result.simulated) {
                            console.log(`📱 SMS de CONEXIÓN simulado para ${user.username} (modo prueba)`);
                        } else {
                            console.log(`📱 SMS de CONEXIÓN enviado para ${user.username}`);
                        }
                    } else if (result.reason === 'throttled') {
                        console.log(`⏸️ SMS de conexión throttled para ${user.username} (muy reciente)`);
                    } else {
                        console.log(`⚠️ No se pudo enviar SMS de conexión para ${user.username}: ${result.reason || result.error}`);
                    }
                })
                .catch(error => {
                    console.error(`❌ Error en SMS de conexión para ${user.username}:`, error);
                });
        }
        ...
        // MODIFICADO: SIEMPRE enviar notificación de conexión, sin importar el estado anterior
        console.log(`👤 ${user.username} ingresó al juego - enviando notificación de CONECTADO`);
        ...
        // Solo notificar a otros jugadores si realmente cambió de estado
        if (!wasConnected) {
...
socket.on('selectTile', async ({ tileIndex, currentScore }) => {
    ...
    // MODIFICADO: Asegurarse de que los valores de punto son precisamente los esperados (20000 en lugar de 30000)
    if (gameState.board[tileIndex].value !== 10000 && gameState.board[tileIndex].value !== -11000) {
        console.error(`VALOR DE FICHA INCORRECTO: ${gameState.board[tileIndex].value}`);
        // Corregir el valor
        gameState.board[tileIndex].value = Math.sign(gameState.board[tileIndex].value) * 10000;
    }
    ...
    if (currentScore !== undefined && Math.abs(currentScore - user.score) > 10000) {
    ...
    // NUEVO: Notificar grandes ganancias y puntajes altos (solo si no es admin)
    if (!user.isAdmin) {
        // Notificar grandes ganancias (si la ganancia es mayor a 20000)
        if (tileValue > 20000) {
            smsNotificationService.notifyBigWin(user.username, tileValue)
                .then(result => {
                    if (result.success) {
                        console.log(`📱 Notificación de gran ganancia enviada para ${user.username}: ${tileValue}`);
                    }
                })
                .catch(error => {
                    console.error(`❌ Error en notificación de gran ganancia:`, error);
                });
        }

        // Notificar puntajes altos (si el puntaje supera 100000)
        if (newScore >= 100000) {
            smsNotificationService.notifyScoreMilestone(user.username, newScore)
                .then(result => {
                    if (result.success) {
                        console.log(`📱 Notificación de puntaje alto enviada para ${user.username}: ${newScore}`);
                    }
                })
                .catch(error => {
                    console.error(`❌ Error en notificación de puntaje alto:`, error);
                });
        }
    }
    ...
    // Guardar usuarios después de cambio de puntaje
    await saveUsers();
});
...
socket.on('rechargePoints', async ({ userId }, callback) => {
    ...
    // Guardar estado
    await saveGameState();
    await saveUsers(); // NUEVO: Guardar usuarios después del cambio
...
// Evento para cambiar el nombre de usuario (MEJORADO)
socket.on('changeUsername', async ({ userId, newUsername }, callback) => {
    ...
    try {
        ...
        // PASO 1: Guardar inmediatamente en archivo local
        await saveUsers();
        console.log(`Archivo users.json actualizado: ${oldUsername} -> ${newUsername}`);

        // PASO 2: Guardar estado del juego
        await saveGameState();
        console.log(`Estado del juego actualizado con nuevo username`);

        // PASO 3: Actualizar en Firebase si está disponible
        if (db) {
            try {
                // Crear un objeto de actualizaciones múltiples para Firebase
                const firebaseUpdates = {};
                
                // Actualizar en la lista de usuarios de Firebase
                firebaseUpdates[`users/${userId}/username`] = newUsername;
                firebaseUpdates[`users/${userId}/lastUsernameChange`] = Date.now();
                
                // Actualizar en gameState/userScores
                firebaseUpdates[`gameState/userScores/${userId}/username`] = newUsername;
                
                // Actualizar en gameState/players si existe
                if (playerIndex !== -1) {
                    firebaseUpdates[`gameState/players/${playerIndex}/username`] = newUsername;
                }

                // Ejecutar todas las actualizaciones de una vez
                await db.ref().update(firebaseUpdates);
                console.log(`Firebase actualizado exitosamente: ${oldUsername} -> ${newUsername}`);
            } catch (firebaseError) {
                console.error('Error al actualizar nombre de usuario en Firebase:', firebaseError);
                // No fallar si Firebase tiene problemas, ya que se guardó localmente
            }
        }

        // PASO 4: Notificar al usuario si está conectado
        const playerSocketId = gameState.players.find(p => p.id === userId)?.socketId;
        if (playerSocketId) {
        ...
        // PASO 5: Actualizar lista de jugadores para todos
        io.emit('playersUpdate', users.filter(u => !u.isAdmin).map(u => ({
            id: u.id,
            username: u.username,
            score: u.score,
        ...
        console.error('Error crítico al cambiar nombre de usuario:', error);
        callback({ success: false, message: 'Error interno del servidor' });
    }
});

// Evento para cambiar la contraseña (MEJORADO)
socket.on('changePassword', async ({ userId, newPassword }, callback) => {
    ...
    try {
        ...
        // PASO 1: Guardar inmediatamente en archivo local
        await saveUsers();
        console.log(`Archivo users.json actualizado: contraseña cambiada para ${targetUser.username}`);

        // PASO 2: Actualizar en Firebase si está disponible
        if (db) {
            try {
                // Crear un objeto de actualizaciones múltiples para Firebase
                const firebaseUpdates = {};
                ...
                // Actualizar en la lista de usuarios de Firebase
                firebaseUpdates[`users/${userId}/password`] = newPassword;
                firebaseUpdates[`users/${userId}/lastPasswordChange`] = Date.now();
                
                // Marcar que la contraseña fue cambiada
                firebaseUpdates[`gameState/userScores/${userId}/passwordChanged`] = true;
                firebaseUpdates[`gameState/userScores/${userId}/passwordChangedAt`] = Date.now();

                // Ejecutar todas las actualizaciones de una vez
                await db.ref().update(firebaseUpdates);
                console.log(`Contraseña actualizada en Firebase para ${targetUser.username}`);
            } catch (firebaseError) {
        ...
        // PASO 3: Notificar al usuario si está conectado
        const playerSocketId = gameState.players.find(p => p.id === userId)?.socketId;
        if (playerSocketId) {
...
socket.on('leaveGame', () => {
    ...
    if (playerIndex !== -1) {
        ...
        // ACTUALIZADO: Notificar desconexión del jugador (solo si no es admin)
        const user = getUserById(userId);
        if (user && !user.isAdmin) {
            ...
            smsNotificationService.notifyPlayerOffline(user.username, "ROYAL")
                .then(result => {
                    if (result.success) {
                        if (result.simulated) {
                            console.log(`📱 SMS de SALIDA simulado para ${user.username} (modo prueba)`);
                        } else {
                            console.log(`📱 SMS de SALIDA enviado para ${user.username}`);
                        }
                    } else if (result.reason === 'throttled') {
                        console.log(`⏸️ SMS de salida throttled para ${user.username} (muy reciente)`);
                    } else {
                        console.log(`⚠️ No se pudo enviar SMS de salida para ${user.username}: ${result.reason || result.error}`);
                    }
                })
                .catch(error => {
                    console.error(`❌ Error en SMS de salida para ${user.username}:`, error);
                });
        }
                ...
                if (!user.isAdmin) {
                    ...
                    smsNotificationService.notifyPlayerOffline(user.username, "ROYAL")
                        .then(result => {
                            if (result.success) {
                                if (result.simulated) {
                                    console.log(`📱 SMS de DESCONEXIÓN simulado para ${user.username} (modo prueba)`);
                                } else {
                                    console.log(`📱 SMS de DESCONEXIÓN enviado para ${user.username}`);
                                }
                            } else if (result.reason === 'throttled') {
                                console.log(`⏸️ SMS de desconexión throttled para ${user.username} (muy reciente)`);
                            } else {
                                console.log(`⚠️ No se pudo enviar SMS de desconexión para ${user.username}: ${result.reason || result.error}`);
                            }
                        })
                        .catch(error => {
                            console.error(`❌ Error en SMS de desconexión para ${user.username}:`, error);
                        });
                }
            }
        }
...
// Configurar guardado periódico más frecuente INCLUYENDO usuarios
setInterval(async () => {
try {
    await saveGameState();
    await saveUsers(); // NUEVO: Guardar usuarios periódicamente
} catch (error) {
...
✅ Sistema funcionando correctamente`;
   ...
   try {
       const result = await smsNotificationService.sendCustomNotification(testMessage);
       res.json({
           success: result.success,
           message: result.success ? 'SMS de prueba enviado correctamente' : 'Error enviando SMS de prueba',
           details: result
       });
   } catch (error) {
       res.status(500).json({
           success: false,
           message: 'Error interno al enviar SMS de prueba',
           error: error.message
       });
   }
});
...
app.get('/', (req, res) => {
res.send('Servidor del juego de memoria (Servidor 4 - juegomemoriaservidor4) funcionando correctamente');
});
...
// Intentar guardar usuarios antes de salir
saveUsers().then(() => {
    console.log('Usuarios guardados de emergencia');
}).catch((err) => {
    console.error('Error al guardar usuarios de emergencia:', err);
});
});
...
try {
    ...
    // Guardar usuarios final
    await saveUsers();
    console.log('Usuarios finales guardados');
